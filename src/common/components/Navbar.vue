<template>
  <div class="navbar">
    <div v-if="data.showOverlay" @click="toggleSidebar" class="overlay"></div>
    ...
    <nav>
      ...
      <label class="logo"><router-link to="/"> Ketofan </router-link></label>
      <ul :class="{ sidebar: data.showOverlay }">
        ...
        <Menu
          v-if="!data.showOverlay && data.showMenu && store.getters.isLogined"
          :show="toggleMenu"
        />
        ...
        <li v-if="store.getters.isLogined && data.showOverlay" @click="logout">
          <span class="material-icons icons"> logout </span>
          Logout
        </li>
      </ul>
    </nav>
